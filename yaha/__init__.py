...
class DICTS:
    LEN = 7
    MAIN, SURNAME, SUFFIX, EXT_STOPWORD, STOPWORD, QUANTIFIER, STOP_SENTENCE = list(
        range(7))
    DEFAULT = ['dict.dic', 'surname.dic', 'suffix.dic',
               'ext_stopword.dic', 'stopword.dic', 'quantifier.dic']
    DEFAULT_DICT = []
        ...
        if not load_ok:
            ...
            try:
                ...
                with open(tmp_file + tmp_suffix, 'wb') as temp_cache_file:
                    pickle.dump(
                        DICTS.DEFAULT_DICT, temp_cache_file, protocol=pickle.HIGHEST_PROTOCOL)
                if os.name == 'nt':
                    import shutil
        ...
        while i < n:
            for j in range(i, min(n, i + self.WORD_MAX), 1):
                if self.exist(sentence[i:j + 1]):
                    graph.add_edge(
                        i, j + 1, 0 - self.get_prob(sentence[i:j + 1]))
                else:
                    # stage3
